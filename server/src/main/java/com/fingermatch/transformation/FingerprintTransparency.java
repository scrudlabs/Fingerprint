...
/**
 * Algorithm transparency API that can capture all intermediate data structures
 * produced by SourceAFIS algorithm. See
 * <a href="https://sourceafis.machinezoo.com/transparency/">algorithm
 * transparency</a> pages on SourceAFIS website for more information and a
 * tutorial on how to use this class.
 * <p>
 * Applications can subclass {@code FingerprintTransparency} and override
 * {@link #log(String, Map)} method to define new transparency data logger. One
 * default implementation of {@code FingerprintTransparency} is returned by
 * {@link #zip(OutputStream)} method.
 * <p>
 * An instance of {@code FingerprintTransparency} must be passed to
 * {@link FingerprintTemplate#transparency(FingerprintTransparency)} or
 * {@link FingerprintMatcher#transparency(FingerprintTransparency)} for
 * transparency data to be actually collected.
 * <p>
 * This class implements {@link AutoCloseable} and callers must ensure that
 * {@link #close()} is called, perhaps by using try-with-resources construct,
 * unless the particular subclass is known to not need any cleanup.
 *
 * @see <a href="https://sourceafis.machinezoo.com/transparency/">Algorithm
 *      transparency in SourceAFIS</a>
...
public abstract class FingerprintTransparency implements AutoCloseable {
	 ...
	 * {@code FingerprintTransparency} is an abstract class. This empty constructor
	...
	/**
	 * Record transparency data. This is an abstract method that subclasses must
	 * override. If algorithm transparency is enabled by passing an instance of
	 * {@code FingerprintTransparency} to
	 * {@link FingerprintTemplate#transparency(FingerprintTransparency)} or
	 * {@link FingerprintMatcher#transparency(FingerprintTransparency)}, this method
	 * is called with transparency data in its parameters.
	 * <p>
	 * Parameter {@code keyword} specifies the kind of transparency data being
	 * logged, usually corresponding to some stage in the algorithm. For
	 * convenience, several related pieces of transparency data are reported
	 * together. All pieces are available via map in parameter {@code data}, keyed
	 * by file suffix identifying the kind of data, usually {@code .json} or
	 * {@code .dat} for JSON and binary data respectively. See
	 * <a href="https://sourceafis.machinezoo.com/transparency/">algorithm
	 * transparency</a> on SourceAFIS website for documentation of the structure of
	 * the transparency data.
	 * <p>
	 * Transparency data is offered indirectly via {@link Supplier}. If this
	 * {@code Supplier} is not evaluated, the data is never serialized. This allows
	 * applications to efficiently collect only transparency data that is actually
	 * needed.
	 * 
	 * @param keyword specifies the kind of transparency data being reported
	 * @param data    a map of suffixes (like {@code .json} or {@code .dat}) to
	 *                {@link Supplier} of the available transparency data
	 * 
	 * @see <a href="https://sourceafis.machinezoo.com/transparency/">Algorithm
	 *      transparency in SourceAFIS</a>
	...
	/**
	 * Release system resources held by this instance if any. Subclasses can
	 * override this method to perform cleanup. Default implementation of this
	 * method is empty.
	 */
	@Override
	public void close() {
	...
	/**
	 * Write all transparency data to a ZIP file. This is a convenience method to
	 * enable easy exploration of the available data. Programmatic processing of
	 * transparency data should be done by subclassing
	 * {@code FingerprintTransparency} and overriding {@link #log(String, Map)}
	 * method.
	 * <p>
	 * The returned {@code FingerprintTransparency} object holds system resources
	 * and callers are responsible for calling {@link #close()} method, perhaps
	 * using try-with-resources construct. Failure to close the returned
	 * {@code FingerprintTransparency} instance may result in damaged ZIP file.
	 * 
	 * @param stream output stream where ZIP file will be written (will be closed
	 *               when the returned {@code FingerprintTransparency} is closed)
	...
	private Supplier<ByteBuffer> json(Supplier<Object> supplier) {
		return () -> ByteBuffer.wrap(
				new GsonBuilder().setPrettyPrinting().create().toJson(supplier.get()).getBytes(StandardCharsets.UTF_8));
	}
	...
	private void log(String name, String suffix1, Supplier<ByteBuffer> supplier1, String suffix2,
			Supplier<ByteBuffer> supplier2) {
		Map<String, Supplier<ByteBuffer>> map = new HashMap<>();
