...
public class HistogramTest {
	private final Histogram h = new Histogram(4, 5, 6);
	...
	@Test public void getCell() {
		assertEquals(125, h.get(new Cell(1, 2), 5));
		assertEquals(243, h.get(new Cell(2, 4), 3));
	}
	@Test public void sumAt() {
		assertEquals(6 * 120 + 1 + 2 + 3 + 4 + 5, h.sum(1, 2));
	}
	@Test public void sumCell() {
		assertEquals(6 * 340 + 1 + 2 + 3 + 4 + 5, h.sum(new Cell(3, 4)));
	}
